...
#define EN_LONG 20   //一次取明文字节数(x,20)(y,20)
...
void main(){
    ...
    GetPrime(&P,P_LONG);//获取素数P，存入P中，长度为P_LONG
	printf("有限域 P 是:\n");	
	char temp[800]={0};
	...
	for(int i=0;i<len;i++)
	{
		...
		//mp_toradix(&m,miwen,10);
		//cout<<"加密的字符串为："<<miwen<<endl;
		Ecc_points_mul(&c1x,&c1y,&GX,&GY,&r,&A,&P);//加密C1=[r]G r为选取的随机数
		Ecc_points_mul(&tempx,&tempy,&QX,&QY,&r,&A,&P); //计算临时变量[r]Q r为选取的随机数 Q点为加密的公钥 
	...
	cout<<"加密结束！"<<endl;
	t_end=clock();
	cout<<"加密运行时间(单位：ms):"<<t_end-t_start<<endl;
	cout<<"请输入密文存放文件（例如：D:\\test密文.txt）：";
	cin>>fout1_name;
...
L3:     miwendraw(&c2, stemp, i);
		...
		//mp_toradix(&m,miwen,10);
		//cout<<"解密后的的字符串为："<<miwen<<endl;


		int chtem;
		chtem=chdraw(&m,stemp);//从ming中取出字符串
...
//打开要加密文件
 ...
 //打开保存密文文件
	...
	{   
  //读入x和y坐标 分别是255的字符长度
	    fread(miwenx,1,EN_LONG,fp);//读入字符串
	    miwenx[EN_LONG]=char(255);

		fread(miweny,1,EN_LONG,fp);//读入字符串
	    miweny[EN_LONG]=char(255);

        putin(&mx, miwenx,EN_LONG+1);//文件存入 		
		putin(&my, miweny,EN_LONG+1);//文件存入
	    ...
	    Ecc_points_mul(&c1x,&c1y,px,py,&r,a,p);//加密C1=[r]G r为选取的随机数
	    Ecc_points_mul(&tempx,&tempy,qx,qy,&r,a,p); //计算临时变量[r]Q r为选取的随机数 Q点为加密的公钥 
