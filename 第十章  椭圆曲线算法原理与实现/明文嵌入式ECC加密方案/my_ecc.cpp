...
void main(){
    ...
    time_t t;
	clock_t t_start,t_en_end,t_de_end;
    srand( (unsigned) time( &t ) );
	t_start=clock();         //程序运行开始计时

	printf("明文嵌入式ECC公钥加密方案！\n");
	...
	t_en_end=clock();
	cout<<"加密文件所用时间(单位:ms):"<<t_en_end-t_start<<endl;      //加密程序运行时间

	printf("------------------------------------------------------------------------\n");

	Ecc_decipher(&K,&A,&P);//解密
	t_de_end=clock();         //程序运行结束计时
	cout<<"解密运行时间总计(单位:ms)："<<t_de_end-t_start<<endl;
...
//打开要加密文件
 ...
 //打开保存密文文件
  ...
  //读入x和y坐标 分别是255的字符长度
	    ...
	    //printf("miwenx 是:");	
        mp_toradix(&mx,miwenx,10);
        //printf("%s\n",miwenx);



	    Ecc_points_mul(&c2x,&c2y,px,py,&r,a,p);//加密C1=[r]G r为选取的随机数
